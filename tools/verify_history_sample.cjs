...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#teacherReviewScreen').isVisible(),true,'Teacher review must open separately');
    assert.equal(await page.locator('.teacher-review-question').count(),17,'Teacher answer review missing');
    assert.match(await page.locator('.teacher-review-question').first().innerText(),/Ответ ученика:/);
    assert.ok(await page.locator('.teacher-review-question.is-wrong .review-options').count()>0,'Wrong answer details missing');
    assert.equal(await page.locator('.teacher-review-question').first().getAttribute('aria-expanded'),'false','Correct task must start compact');
    await page.locator('.teacher-review-question').first().click();
    assert.equal(await page.locator('.teacher-review-question').first().getAttribute('aria-expanded'),'true','Task did not expand');
    assert.ok(await page.locator('.teacher-review-question').first().locator('.review-options').count()>0,'Expanded task options missing');
